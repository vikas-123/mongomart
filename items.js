...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var totalCollections = this.db.collection('item').count();
        ...
        //result.unshift({_id: "All",
          // num: totalCollections});
        var result1=[];
        result1.push(category);
        for(var pq=0;pq<result.length;pq++){
            result1.push(result[pq]);
        }
        callback(result1);
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if(category=="All")
        {
             cursor = this.db.collection('item').find({}).skip(toSkip).limit(itemsPerPage);

        }
        else{

        cursor = this.db.collection('item').find({category:category}).skip(toSkip).limit(itemsPerPage);
        }
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         *
         */
         var cursor=this.db.collection('item').find({ $text: { $search:  query} }).skip(page*5).limit(itemsPerPage);
